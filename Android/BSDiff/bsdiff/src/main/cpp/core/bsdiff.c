...
int executeDiff(int argc, char *argv[])
{
    ...
    if (fseek(fs, 0, SEEK_END) != 0)
        err(1, "Seek failed :%s", argv[1]);
    oldsize = ftell(fs);
    pold = (u_char *)malloc(oldsize + 1);
    if (pold == NULL)
        err(1, "Malloc failed :%s", argv[1]);
    fseek(fs, 0, SEEK_SET);
    if (fread(pold, 1, oldsize, fs) == -1)
        err(1, "Read failed :%s", argv[1]);
    if (fclose(fs) == -1)
        err(1, "Close failed :%s", argv[1]);
    ...
    fs = fopen(argv[2], "rb");
    if (fs == NULL)
        err(1, "Open failed :%s", argv[2]);
    if (fseek(fs, 0, SEEK_END) != 0)
        err(1, "Seek failed :%s", argv[2]);
    newsize = ftell(fs);
    pnew = (u_char *)malloc(newsize + 1);
    if (pnew == NULL)
        err(1, "Malloc failed :%s", argv[2]);
    fseek(fs, 0, SEEK_SET);
    if (fread(pnew, 1, newsize, fs) == -1)
        err(1, "Read failed :%s", argv[2]);
    if (fclose(fs) == -1)
        err(1, "Close failed :%s", argv[2]);
    ...
    while (scan < newsize)
    {
        ...
        if ((len != oldscore) || (scan == newsize))
        {
            ...
            {
                if (pold[lastpos + i] == pnew[lastscan + i])
                    s++;
                i++;
                if (s * 2 - i > Sf * 2 - lenf)
                {
